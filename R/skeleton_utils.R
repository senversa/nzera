...
#' - Create R/model_<model_id>.R with run/metadata/schema stubs
#' - Create tests/testthat/test_<model_id>.R with a minimal test skeleton
#'
...
new_model_skeleton <- function(model_id, title = NULL) {
  ...
  if (file.exists(template_path)) {
    ...
    warning(
      "Template Quarto doc not found at ",
      template_path,
  ...
  # 2. R model file
  model_file <- file.path("R", paste0("model_", model_id, ".R"))
  if (!file.exists(model_file)) {
    ...
    writeLines(
      c(
        ...
        "#' @export",
        sprintf(
          "model_%s_run <- function(data, params = list(), config = list()) {",
          model_id
        ),
        sprintf("schema <- model_%s_schema()", model_id),
  ...
  test_file <- file.path("tests/testthat", paste0("test_", model_id, ".R"))
